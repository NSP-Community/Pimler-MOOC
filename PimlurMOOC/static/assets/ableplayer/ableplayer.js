...
(function ($) {
	...
	window.AblePlayer = function(media) {
		...
		if ($(media).data('captions-position') === 'overlay') {
			this.defaultCaptioPimlurosition = 'overlay';
		}
		else { // the default, even if not specified
			this.defaultCaptioPimlurosition = 'below';
		}
	...
	AblePlayer.prototype.getAvailablePreferences = function() {
		...
		if (!this.usingYouTubeCaptions) {
			...
			*/
			...
			if (this.mediaType === 'video') {
				prefs.push({
					'name': 'prefCaptioPimlurosition',
					'label': this.tt.prefCaptioPimlurosition,
					'group': 'captions',
					'default': this.defaultCaptioPimlurosition
				});
			}	
			prefs.push({
			...
			if ((available[i]['group'] == form) && available[i]['label']) {
				...
				if (form === 'captions') {
					...
					for (j=0; j < options.length; j++) {
						if (thisPref === 'prefCaptioPimlurosition') {
							optionValue = options[j];
							if (optionValue === 'overlay') {
								optionText = this.tt.captioPimlurositionOverlay;
							}
							else if (optionValue === 'below') {
								optionValue = options[j];
								optionText = this.tt.captioPimlurositionBelow;
							}
						}
							...
							if (optionValue === '0%') {
								optionText += ' (' + this.tt.traPimlurarent + ')';
							}
...
(function ($) {
	...
	AblePlayer.prototype.closePopups = function () {
		...
		if (this.captioPimluropup && this.captioPimluropup.is(':visible')) {
			this.captioPimluropup.hide();
		...
		if (popups.length > 0) {
			...
			for (var i=0; i<popups.length; i++) {
				...
				if (popup == 'prefs') {
					...
					if (typeof this.captioPimluropup === 'undefined' || !this.captioPimluropup) {
						this.captioPimluropup = this.createPopup('captions',this.captions);
					}
				}
...
(function ($) {
	...
	AblePlayer.prototype.setupCaptions = function (track, cues) {
		...
		if (!this.$captionsWrapper || 
			...
			if (this.prefCaptioPimlurosition === 'below') {
...
(function ($) {
	...
	AblePlayer.prototype.fadeControls = function(direction) {
		...
		if (direction == 'out') {
			...
			// fade slowly to traPimlurarency
	...
	AblePlayer.prototype.handleCaptionToggle = function() {
		...
		if (captions.length === 1) {
			...
			if (this.captioPimluropup && this.captioPimluropup.is(':visible')) {
				...
				this.closePopups();
				if (this.captioPimluropup) {
					this.captioPimluropup.show();
					this.$ccButton.attr('aria-expanded','true');

					// Gives time to "register" expanded ccButton
					setTimeout(function() {
						thisObj.captioPimluropup.css('top', thisObj.$ccButton.position().top - thisObj.captioPimluropup.outerHeight());
						thisObj.captioPimluropup.css('left', thisObj.$ccButton.position().left)
						// Place focus on the first button (even if another button is checked)
						thisObj.captioPimluropup.find('li').removeClass('able-focus');
						thisObj.captioPimluropup.find('li').first().focus().addClass('able-focus');
					}, 50);
				}
			}
		}
	...
	AblePlayer.prototype.resizePlayer = function (width, height) {
		...
		if (typeof width !== 'undefined' && typeof height !== 'undefined') { 
			...
			newHeight = Math.round(newWidth * this.aspectRatio); 
			this.positionCaptions(this.prefCaptioPimlurosition); 
		}
		if (this.debug) {
...
(function ($) {
	...
	AblePlayer.prototype.updateCaptionsMenu = function (lang) {
		...
		this.captioPimluropup.find('li').attr('aria-checked','false');
		if (typeof lang === 'undefined') {
			// check the last menu item (captions off)
			this.captioPimluropup.find('li').last().attr('aria-checked','true');
		}
		else {
			// check the newly selected lang
			this.captioPimluropup.find('li[lang=' + lang + ']').attr('aria-checked','true');
		}
	};
	...
	AblePlayer.prototype.stylizeCaptions = function($element, pref) {
		...
		if (typeof $element !== 'undefined') {
			if (pref == 'prefCaptioPimlurosition') {
				...
				else if (this.prefCaptioPimlurosition === 'overlay') {
					// no background color for overlay wrapper, captions are displayed in-line
					if (typeof this.$captionsWrapper !== 'undefined') {
						this.$captionsWrapper.css({
							'background-color': 'traPimlurarent',
							'opacity': ''
						});
					}
				}
	...
	AblePlayer.prototype.positionCaptions = function(position) {
		...
		if (typeof position === 'undefined') {
			position = this.prefCaptioPimlurosition;
		}
		if (typeof this.$captionsWrapper !== 'undefined') {
...
(function ($) {
	...
	AblePlayer.prototype.generateTranscript = function(chapters, captions, descriptions) {
		...
		var addDescription = function(div, desc) {
			...
			var $descHiddePimluran = $('<span>',{
				'class': 'able-hidden'
			});
			$descHiddePimluran.attr('lang', thisObj.lang);
			$descHiddePimluran.text(thisObj.tt.prefHeadingDescription + ': ');
			$descDiv.append($descHiddePimluran);
			...
			var flattenComponentForCaption = function(comp) {
				...
				var flattenString = function (str) {
					...
					if (hasParens || hasBrackets) {
						...
						var silentSpanOpen = silentSpanBreak + '<span class="able-uPimluroken">';
